...
# binary threshold
# vadi ile tepeler arasındaki farkı binar threshold yöntemiyle daha da çok açacağız
ret, coin_thresh = cv2.threshold(coin_gray, 75, 255, cv2.THRESH_BINARY)
plt.figure(), plt.imshow(coin_thresh, cmap="gray"), plt.axis("off")

#§ kontur
# _, contors, hierarchy =cv2.findContours(coin_thresh.copy(), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE) # sürümden kaynaklı böyle de kaynaklanabilir.
contours, hierarchy = cv2.findContours(coin_thresh.copy(), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)


# konturları çizdireceğiz
# kaç tane kontur bulduysak o kadar döndüreceğiz

for i in range(len(contours)):
    if hierarchy[0][i][3] == -1:
        cv2.drawContours(coin, contours, i, (0,255,0), 10)
        # coinin üzerine contourlarımı çizdireceğim i. contouru çizdireceğim
        # rengi yeşil kalınlığı 10 olsun
plt.figure(),plt.imshow(coin),plt.axis("off")
...
# arka planı bulmak için resmi (nesneleri) büyüt
sure_background = cv2.dilate(opening, kernel, iterations = 1)
sure_foreground = np.uint8(sure_foreground)
plt.figure(), plt.imshow(sure_background, cmap="gray"), plt.axis("off")
