...
id1 = 29
# 29 etiketli aracı takip edeceğiz
numberOfImage = np.max(data["frame_number"])
# 750 frameden oluşun videonun max frame'ini alıyoruz.
fps = 25
bound_box_list = []
...
for i in range(numberOfImage-1):
    ...
    if ret:
        ...
        # eğer aracımızın uzunluğu boş değilse
        if len(car[filter_id1]) != 0:
            ...
            # merkeze noktamızı çizdiriyoruz.
            
            bound_box_list.append([i, x, y, w, h, int(x+w/2), int(y+h/2)])
            # i=frame,x ve y koordinatları, genişilik, yükseklik, center_x,center_y
            
        cv2.putText(frame, "Frame num:"+str(i+1), (10,30), cv2.FONT_HERSHEY_SCRIPT_COMPLEX, 1, (0,0,255), 2)
